...
public class MixedDustNameRegistry {
	...
	private static String getDefaultName(int aL, int bL, int cL) {
		...
		boolean first = true;
		if ( aL > 0 ) {
			builder.append(getALevel(aL,first));
			first = false;
		}
		if ( bL > 0 ) {
			builder.append(getBLevel(bL,first));
			first = false;
		}
		if ( cL > 0 ) {
			builder.append(getCLevel(cL,first));
		}
		builder.append(" (");
		if ( aL > 0 ) {
			builder.append(aL+"A");
		...
		return Character.toUpperCase(builder.toString().charAt(0)) + builder.toString().substring(1);
	}
	...
	private static String getALevel(int aL, boolean first) {
		...
		String name = "axi";
		if ( num.length() > 0 && num.substring(num.length()-1, num.length()).equals("a") )
			name = "xi";
	...
	private static String getBLevel(int bL, boolean first) {
		...
		String name = "ogen";
		if ( num.length() > 0 && num.substring(num.length()-1, num.length()).equals("o") )
			name = "gen";
