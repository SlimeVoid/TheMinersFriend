...
import slimevoid.tmf.client.renderers.models.ModelTMFObject;

public class WavefrontOBJModelLoader {
	private static DecimalFormat df = new DecimalFormat("#.####");
	private static Map<File,ModelTMFObject> cache = new HashMap<File,ModelTMFObject>();
	
	
	public ModelTMFObject loadObjFile(ModelBase baseModel, int texW, int texH, File file) {
		if ( cache.containsKey(file) )
			return cache.get(file);
		
		ModelTMFObject out = new ModelTMFObject((new ModelRenderer(baseModel, 0, 0)).setTextureSize(texW, texH));
	...
	private void parse(String objStr, ModelTMFObject objModel) {
	...
	private void parseLine(String line, ModelTMFObject objModel) {
